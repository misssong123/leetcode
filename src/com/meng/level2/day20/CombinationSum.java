...
public class CombinationSum {
    ...
    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        fillData(0,target,0,candidates,list,len);
        return res;
    }
    private void fillData(int index, int target, int sum, int[] candidates, List<Integer> list,int len) {
        ...
        if (sum + candidates[index] <= target){
            ...
            fillData(index,target,sum+candidates[index],candidates,list,len);
            list.remove(list.size()-1);
        }
        //不放当前数据
        fillData(index+1,target,sum,candidates,list,len);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(demo.combinationSum(candidates,target));
    }
    /**
     * 方法一：搜索回溯
     * 思路与算法
